...
console.log('PremadeItinerary model schema:', PremadeItinerary.schema.paths.plans);
...
const allItinerariesData = itinerariesData;
...
async function seedItineraries() {
  try {
    ...
    await PremadeItinerary.deleteMany({});
    console.log('Existing data cleared');
    
    let successCount = 0;
    for (let i = 0; i < itinerariesData.length; i++) {
      const city = itinerariesData[i];
      try {
        const cityDoc = new PremadeItinerary({
        ...
        await cityDoc.save();
        successCount++;
        console.log(`✅ ${city.cityName} inserted successfully`);
      } catch (error) {
        console.error(`❌ Error inserting ${city.cityName}:`, error.message);
      }
    }
    
    console.log(`${successCount}/${itinerariesData.length} itineraries seeded successfully!`);
  } catch (error) {
...
console.log('Starting seeder...');
console.log('MongoDB URI:', process.env.MONGODB_URI);
console.log('Total cities to insert:', allItinerariesData.length);
